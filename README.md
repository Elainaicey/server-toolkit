...
| 系统管理 | 环境检查、主机名、时区、Swap、时间同步、软件包健康、磁盘分析、安全清理 |
| 网络与端口 | 地址与路由、DNS、IPv4/IPv6 连通性、监听端口、端口进程、BBR、地址优先级 |
| 安全中心 | UFW 状态与规则、保留 SSH 端口的防火墙向导、SSH 安全向导、登录事件 |
| 服务与日志 | 运行/失败服务、状态、Journal 日志、启动停止、重启、开机启动、操作记录 |
| Docker | 概览、容器、镜像、日志、检查、启动停止和重启；未安装时可单独安装 |
...
卸载只删除确认属于本项目的程序目录和命令链接，不删除配置备份和操作日志。
